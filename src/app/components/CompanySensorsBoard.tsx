    ...
    <div className="flex h-full min-h-0 flex-col gap-4">
      <div className="grid shrink-0 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {(
          [
            ...
            { key: "repair_request" as const, label: "????" },
            { key: "return_request" as const, label: "????" },
          ] as const
        ).map((item) => (
          <button
            key={item.key}
      ...
      <Panel className="flex min-h-0 flex-1 flex-col !p-4">
        <div className="mb-3 flex shrink-0 flex-wrap items-center justify-between gap-3">
          <div>
            <div className="text-lg text-[var(--eh-mist)]">?? ?? ??</div>
        ...
        <div className="mt-3 flex shrink-0 items-center justify-between gap-3 text-xs text-[var(--eh-fog)]">
          <span>
            {filtered.length}? ? {safePage * pageSize + 1}?
